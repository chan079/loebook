...
주의할 점이 있다. 지금 이 실습에서는 2016년 사망률 자료가 있어서 예측의
성능을 평가해 볼 수 있지만, 실제 적용 시에는 성능을 확인할 방법이
없다. 그러므로 1차함수가 좋은지 knot가 1개씩 있는 NS가 좋은지 아니면
다른 것이 좋은지 바로 알 방법이 없다. 시간이 지나서 미래가 과거로
변하면 그때서야 판단할 수 있다.

마지막으로, R의 `ns`(자연 3차 스플라인)와 `bs`(스플라인) 함수는 끝점과
중간 구분점 등을 변수값 분포에 따라 자동으로 설정하지만, `predict`를
할 때에는 학습용 데이터에서 결정한 정보를 테스트용 데이터에도 그대로
사용한다. 이 점은 매우 considerate하다.
