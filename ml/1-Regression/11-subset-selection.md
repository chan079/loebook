...
위에서 그룹핑을 하는 방법이 좀 복잡해 보이는데, 10개로 분할할 때 개수를 최대한 균등하게 하기 위해서 그렇게 한 것이다. 안쪽에 `rep(1:10, length.out=nrow(z14))`는 1부터 10까지를 총 `n`개가 되도록 재활용하고, 바깥쪽에 `sample()` 함수는 이것을 무작위로 섞는다.
